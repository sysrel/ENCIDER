...
/* SYSREL extension */
void StatsTracker::stepInstructionThread(ExecutionState &es, int tid) {
  if (OutputIStats) {
    if (TrackInstructionTime) {
      static sys::TimeValue lastNowTime(0,0),lastUserTime(0,0);
    ...
    Instruction *inst = es.threads[tid].pc->inst;
    const InstructionInfo &ii = *es.threads[tid].pc->info;
    StackFrame &sf = es.threads[tid].stack.back();
    theStatisticManager->setIndex(ii.id);
    if (UseCallPaths)
      theStatisticManager->setContext(&sf.callPathNode->statistics);

    //llvm::outs() << sf.callPathNode->statistics.getValue() << "\n";
...
/* SYSREL extension */
/* Should be called _after_ the es->popFrameThread() */
void StatsTracker::framePoppedThread(ExecutionState &es, int tid) {
  // XXX remove me?
}
/* SYSREL extension */
...
void StatsTracker::updateStateStatistics(uint64_t addend) {
  for (std::set<ExecutionState*>::iterator it = executor.states.begin(),
    ...
    if (asyncMode) {
    if (state.rtid == -1) {
      if (!state.getWaitingForThreadsToTerminate()) { // otherwise the pc is invalid
      ...
      if (!state.threadTerminated(state.rtid)) { // otherwise the pc is invalid
       const InstructionInfo &ii = *state.threads[state.rtid].pc->info;
       theStatisticManager->incrementIndexedValue(stats::states, ii.id, addend);
       if (UseCallPaths)
         state.threads[state.rtid].stack.back().callPathNode->statistics.incrementValue(stats::states, addend);
      }
    }
   }
   else {
