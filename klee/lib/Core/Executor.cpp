...
std::fstream datacacheos;
std::fstream secretdependentos;
std::fstream timingos;
std::fstream normaltermtimingos;
std::fstream inferredtimingos;
std::fstream codecacheos;
...
std::map<long, std::map<long, std::map<std::string, 
                    std::map<unsigned, std::set<long> > > > > timingObsPointsMap;
...
std::string getSouceWithContext(ExecutionState &state, KInstruction *ki) {
   ...
   if (EncVer) llvm::errs() << "stack trace=" << swctx << "\n";
...
cl::opt<bool>
UseInternal("use-internal", cl::desc("Try to locate the renamed version of a function that is declared as internal"), cl::init(false));
...
cl::opt<bool>
HandleSingleSuccessors("handle-single-succ", 
    cl::desc("Check security sensitive branches when single successor\n"), 
    cl::init(false));
...
Function *getInternalVersion(std::string fname) {
   for(unsigned int i=0; i < kmoduleExt->functions.size(); i++) {
      Function *f = kmoduleExt->functions[i]->function;
      if (f) {
...
void pushInfoFlowContext(ExecutionState &state) {
   ...
   currentInfoFlowContext = initvalue;
   if (EncVer) llvm::errs() << "pushing info flow context for " << &state << "\n";
}
...
void popInfoFlowContext(ExecutionState &state) {
   ...
   if (EncVer) llvm::errs() << "poping info flow context for " << &state << "\n";
}
...
void saveInfoFlowContext(ExecutionState &state) {
   ...
   int last = infoflowstack[&state]->size() - 1;
   (*infoflowstack[&state])[last] = currentInfoFlowContext;       
   if (EncVer) llvm::errs() << "saving info flow context for " << &state << "\n"; 
}
...
void Executor::updateInfoFlowForLoad(ExecutionState &state, 
   ...
   if (!CE) { 
      ...
      offset = 0;
      size = mo->size;
      if (EncVer) llvm::errs() << "handling symbolic offset by conservatively using 0 and " << mo->size << "for size\n"; 
   }
   ...
   if (EncVer) llvm::errs() << "Info Flow For LOAD register=" << regIndex << " offset=" << offset << " size=" << size << "\n";
   std::map<const MemoryObject*, 
                  std::map<region, 
   ...
   if (notupdated.size() > 0) {
      if (EncVer) llvm::errs() << "Trying to fill uncovered " << notupdated.size() << " regions in LOAD after handling flow from memory\n";
      ...
      if (source >= 0) {
         ...
         std::map<region, std::set<InfoFlowRegion_t> > ifrmap  = (*currentInfoFlowContext.localIFlow)[target->dest];
         for(auto rtp : notupdated) {
            if (EncVer) llvm::errs() << "region " << rtp.offset << ":" << rtp.size << "\n";
...
InfoFlowRegion_t adjust(InfoFlowRegion_t ifr, region adj) {
  if (EncVer) llvm::errs() << "adj = " << adj.offset << "," << adj.size << "\n";
...
void Executor::updateInfoFlowForStore(ExecutionState &state, 
   ...
   if (EncVer) llvm::errs() << "Info Flow For STORE register=" << regIndex << "\n";
   int dest = isInfoFlowRelevant(target, destregIndex); 
   if (dest >= 0) {
      // info flow tracking relevant
      destif = createInfoFlowRegion(dest, offset, size);  
      if (EncVer) llvm::errs() << "Adding info flow relevant destination in STORE: " << target->inst << "\n";
      destifs = setIF(destif);
      if (EncVer) print(destifs);
      record = true;
   }
   ...
   if (source >= 0  || currentInfoFlowContext.localIFlow->find(regIndex) != currentInfoFlowContext.localIFlow->end()) {
      ...
      if (source >= 0) {
         ...
         if (EncVer) llvm::errs() << "Adding info flow relevant source in STORE: " << target->inst << "\n";
         sourceifs = setIF(sourceif);
         if (EncVer) print(sourceifs);
      ...
      if (EncVer) llvm::errs() << "CLEARING INFO FLOW\n";
...
void Executor::updateInfoFlowForReturn(ExecutionState &state, int regIndex, KInstruction *ki) {
   if (EncVer) llvm::errs() << "Info Flow for RETURN\n";
   if (!infoFlowSummaryMode) return;
   if (regIndex < 0) return; 
...
bool isInputFunction(Function *function) {
    ...
    if (EncVer) llvm::errs() << "checking " << fname << " to see if an input function\n";
...
void timeSideChannelAnalysis(Executor *executor) {
 ...
 for(auto im : timingObsPointsMap) {  
   ...
   for(auto topm : im.second) {
    ...
    for(auto frs : topm.second) {
       std::vector<unsigned> succMin;
       std::vector<unsigned> succMax;
       std::vector<long> maxRD;
       std::vector<long> minRD;
       //int maxsucc = -1, minsucc = -1;
       for(auto succ : frs.second) {
         long int min=LONG_MAX, max=0;
         long minrd, maxrd;
         for(auto rdd : succ.second) {
             if (((RD*)rdd)->ru < min) { 
                min = ((RD*)rdd)->ru;
                minrd = rdd;
             }
         ...
         succMin.push_back(min);
         succMax.push_back(max);
         minRD.push_back(minrd);
         maxRD.push_back(maxrd);
      }
      for(unsigned int i=0; i<succMin.size(); i++) {
         for(unsigned int j=i+1; j<succMin.size(); j++) {
            int diff1 = succMax[i] - succMin[j];
            int diff2 = succMax[j] - succMin[i]; 
            if (diff1 >= maxdiff || 
                diff2 >= maxdiff) {
               if (diff1 > diff2) {
                  maxdiff = diff1;
                  ...
                  if (diff1 > maxdiff)
                     topsCandidate.clear();
                  topsCandidate.insert(maxRD[i]); 
                  topsCandidate.insert(minRD[j]); 
               }
               else if (diff1 < diff2) {
                  maxdiff = diff2;
                  //maxsucc = j;
                  ...
                  if (diff2 > maxdiff)
                     topsCandidate.clear();
                  topsCandidate.insert(maxRD[j]); 
                  topsCandidate.insert(minRD[i]); 
               }
               else {
                  maxdiff = diff2;
                  //maxsucc = j;
    ...
    if (EncVer) llvm::errs() << "Timing observation point for H-ancestor " << (*((Instruction*)im.first)) 
                 << fname << "\n";

   for(auto t : topsCandidate) { 
      finalTops.insert(t);
      if (EncVer) llvm::errs() << "RD with usage " << ((RD*)t)->ru << "\n";
   }
 ...
 codecacheos.rdbuf()->pubsetbuf(0, 0);
 codecacheos.open("codeCacheSideChannels_time.txt", std::fstream::out);
 timingos.rdbuf()->pubsetbuf(0, 0);
 timingos.open("timingSideChannels_time.txt", std::fstream::out);
 normaltermtimingos.rdbuf()->pubsetbuf(0, 0);
 normaltermtimingos.open("normalTermTimingSideChannels_time.txt",                                              std::fstream::out);
 inferredtimingos.rdbuf()->pubsetbuf(0, 0);
 inferredtimingos.open("inferredTimingSideChannels_time.txt", 
                                            std::fstream::out);
 ...
 codecacheos.close();
 timingos.close();
 normaltermtimingos.close();
 inferredtimingos.close();       
 if (EncVer) llvm::errs() << "closed timing stat files!\n";
...
Type *getTypeFromName(llvm::Module *module, std::string tname) {
   ...
   for (auto *STy : StructTypes) { 
    if (EncVer) llvm::errs() << getTypeName(STy) << " vs " << tname << "\n";
...
void Executor::checkHighSensitiveLocals(ExecutionState &state, Instruction *ii) {
  ...
  if (EncVer) llvm::errs() << "checking leak in " << sf.kf->function->getName() << " with " << sf.allocas.size() << "locals \n";
  ...
  for (std::vector<const MemoryObject*>::iterator it = sf.allocas.begin(), 
      ...
      if (checkLeaksPreciseAndFocused) {
         ...
         if (stackLeakToBeChecked.find(cfname) != stackLeakToBeChecked.end() && 
            ...
            if (EncVer) llvm::errs() << "Sensitive leak via local variable " << localVar << " in function " << fname << " at address " << (*it)->getBaseExpr() << "\n"; 
         }
      }
   ...
   if (EncVer) llvm::errs() << "checking done\n";
}
     ...
     for(llvm::Function::arg_iterator agi = function->arg_begin(); agi != function->arg_end(); agi++, a++) {
        ...
        if (at->isPointerTy()) {
           ...
           if (asuccess) {
              ...
              if (EncVer) llvm::errs() << "checking arg " << a << " of " <<  fname << " : " << result << " for sensitive flow\n"; 
              if (exprHasSymRegion(state, result, true)) {
                 if (EncVer) llvm::errs() << " high region flows into " << fname << "\n";
...
void clearMemoryRegion(ExecutionState &state, ref<Expr> a, std::vector<region> rs, bool high) {
   ...
   if (m.find(a) != m.end()) {
      ...
      if (temp.size() == 0) {
         m.erase(a);
         if (EncVer) llvm::errs() << "clearing high=" << high << " status of " << a << "\n";
      }
      else m[a] = temp;
      if (high) 
  ...
  if (high && highSymRegions.find(name) != highSymRegions.end()) {
     ...
     clearRegion(rs, cur, temp);
      if (temp.size() == 0) {
         ...
         if (EncVer) llvm::errs() << "clearing high=" << high << " status of " << name << "\n";
      }
...
void setHighSymRegion(std::string name, std::vector<region> rs) {
  ...
  if (EncVer) llvm::errs() << "setting symbolic region high: " << name << "\n";
}
...
void setLowSymRegion(std::string name, std::vector<region> rs) {
  ...
  if (EncVer) llvm::errs() << "clearing " << name << " from high\n";
...
bool Executor::isInRegion(ExecutionState &state, std::vector<region> rs, ref<Expr> offset, Expr::Width offsetType, Expr::Width type, bool computed) {
  if (klee::ConstantExpr *CE = dyn_cast<klee::ConstantExpr>(offset)) {
    ...
    if (EncVer) llvm::errs() << "Symbolic offset do not fall into the security sensitive region!\n";
  }
  return false;
}
...
bool Executor::isInHighMemoryRegion(ExecutionState &state, ref<Expr> baseAddress, ref<Expr> offset, Expr::Width offsetType, Expr::Width type, bool computed) {
  ...
  if (m.find(baseAddress) == m.end()) {
     if (EncVer) llvm::errs() << "base address not marked as security sensitive\n";
     return false;
  }
  else {
     std::vector<region> rs = m[baseAddress];
     if (EncVer) llvm::errs() << "checking if is in high mem region: \n";
     return isInRegion(state, rs, offset, offsetType, type, computed);
  }
}
...
void Executor::checkAndUpdateInfoFlow(ExecutionState &state, Function *f, std::vector<ref<Expr> > & args, 
   ...
   if (argsValues.size() == 0) {
      if (EncVer) llvm::errs() << "no relevant args to check for info flow for " << fname << "\n";   
      return;
   }
   ...
   if (rs.size() > 0) {
      ...
      if (EncVer) llvm::errs() << "calling setHighSym from checkAndUpdateInfoFlow for " << fname << "\n"; 
     ...
     for (auto *STy : StructTypes) {
        ...
        if (tname.find(vct) != std::string::npos) 
           if (targetData->getTypeStoreSize(STy) > maxVoidTypeCastSize) {
              if (EncVer) llvm::errs() << "Storing size of " << tname << " for max void cast size: " << targetData->getTypeStoreSize(STy) << " \n";
  ...
  if (st) {
     ...
     if (lazyInitInitializersWValues.find(tname) != lazyInitInitializersWValues.end()) {
       //llvm::errs() << "initializer found! for type " << tname << "\n"; 
        ...
        for(auto offset : offsets) {
           ...
           if (EncVer) llvm::errs() << "Initializing object of lazy init type " << tname << " at offset " << offset.first << " to value " << ve << " at base address " << mo->getBaseExpr() << "\n";
     ...
     if (lazyFuncPtrInitializers.find(tname) != lazyFuncPtrInitializers.end()) {
        ...
        for(auto e : fmap) {
           ...
           if (!f) {
              if (EncVer) llvm::errs() << "Unable to find function " << e.second << " in the module to init func pointer!!!\n";
              exit(1);
           }
           // assign the function address to the offset
           obj->write(e.first, Expr::createPointer((uint64_t)f));
           if (EncVer) llvm::errs() << "assigning function " << f->getName() << "to offset " << e.first << " of base address " << mo->getBaseExpr() << "\n";
        }
     }
  }
}
         ...
         if (cexpr) {
            ...
            if (asuccess) {
               if (mayBeUnitializedSymbolic(op.second,getWidthForLLVMType(bt))) { 
                  ...
                  if (EncVer) llvm::errs() << op.second->isByteConcrete(0) << " " 
                               << op.second->isByteKnownSymbolic(0) << " "
                               << op.second->isByteFlushed(0) << "\n";
                  if (EncVer) llvm::errs() << "getArgValue " << ai  << "\n";
  ...
  if (const ConstantVector *cp = dyn_cast<ConstantVector>(c)) {
   ...
   // assert(StoreBits >= C->getWidth() && "Invalid store size!");
    if (StoreBits > C->getWidth()) {
      if (EncVer) llvm::errs() << "WARNING: StoreBits " << StoreBits << " greater than width " << C->getWidth() << "\n";
      C = C->ZExt(StoreBits);
    }
    os->write(offset, C);
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal, bool forBranch) {
    ...
    if (replayPath && !isInternal) {
      ...
      if (res==Solver::True) {
        ...
        if(branch) {
          res = Solver::True;
          if (EncVer) { 
             llvm::errs() << "Last condition before "
                               << "fork single successor: ";
             llvm::errs() <<  current.lastCondition << "\n";
          }
  ...
  if (isSeeding &&
    ...
    if (!(trueSeed && falseSeed)) {
      ...
      if (EncVer) { 
         llvm::errs() << "Last condition after "
                               << "fork single successor: ";
         llvm::errs() <<  current.lastCondition << "\n";
      }
...
void Executor::executeCall(ExecutionState &state,
  ...
  if (f && f->isDeclaration()) {
    ...
    if (InvokeInst *ii = dyn_cast<InvokeInst>(i))
      ...
      if (proseFn) {
         if (EncVer) llvm::errs() << "WARNING: Using " << prosename << " for " << f->getName() << "\n";
       ...
       /*if (UseInternal) {
          Function *inf = getInternalVersion(f->getName().str());
          if (inf) {
             llvm::errs() << "WARNING: Using internal function " << inf->getName() << " for " << f->getName() << "\n";
             ((CallInst*)ki->inst)->setCalledFunction(inf);
             executeCall(state, ki, inf, arguments);   
             return;
          }
       }*/
    ...
    if (EncVer) llvm::errs() << "before check 2\n";
    symbolizeAndMarkSensitiveArgumentsOnCall(state,ki,f,arguments,false);
    if (EncVer) llvm::errs() << "after check 2\n";
  }
}
...
int reachableBySuccessor(RD *r1, RD *r2) {
    int sno = 0;
    for(auto rs : (*r2->succ)) {
       if  (reachable(r1, rs))
           return sno;
...
void Executor::recordTimingObservationPoint(void* rddtermp, void *child, void *hap, std::string fname) {
   if (!hap || fname == "") 
      return;
   RD* rddterm = (RD*)rddtermp;
   RD* ha = (RD*)hap;
   if (!ha->HA) {
      unsigned sucno = reachableBySuccessor((RD*)rddtermp, ha);
      assert(sucno != -1 && "Couldn't match the successor of RD node!\n");
      std::map<long, std::map<std::string, std::map<unsigned, std::set<long> > > > m1;
      std::map<std::string, std::map<unsigned, std::set<long> > > m2;
      std::map<unsigned, std::set<long> >m3;
      std::set<long> s;
     ...
     if (EncVer) llvm::errs() << "Successor " << sucno << " of h ancestor at " << (*ha->i) << "\n";
   }
   else recordTimingObservationPoint(rddterm, hap, ha->HA, fname);
}
...
void Executor::handleTimingObservationPoint(ExecutionState &state, bool restart, std::string fname) {
   ...
   if (EncVer) llvm::errs() << "copy of state " << &state << " to be terminated: " << tobeterm << "\n"; 
   ...
   if (EncVer) llvm::errs() << "Candidate tops " << rddterm->stateid << " with inst " << (*rddterm->i) << "\n";
   recordTimingObservationPoint(rddterm, rddterm, rddterm->HA, fname);
   /*if (rddterm->HA && fname != "") {
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  if (EncVer) llvm::errs() << "next instruction: " << (*i) << "\n";

  /* SYSREL extension */
  /* side channel begin */
  if (!state.inEnclave && i->getParent()->getParent() && 
         ((Function*)i->getParent()->getParent())->getName() == state.getLastEnclaveFunction()) {
     if (EncVer) llvm::errs() << "returned back to the enclave inside " << state.getLastEnclaveFunction() << "\n";
  ...
  case Instruction::Ret: {
    ...
    if (state.stack.size() <= 1) {
      ...
      if (!state.activeThreads() && (!state.hasLCM() || state.lcmCompletesWith(rf->getName().str()))) {
         if (EncVer) llvm::errs() << "terminating state with " << rf->getName() << "\n";
      ...
      bool isSExt = cs.paramHasAttr(0, llvm::Attribute::SExt);
        ...
        //}
        #ifdef VB
        if (EncVer) llvm::errs() << "Warning: return void when caller expected a result\n";
        #endif
        /* SYSREL extension */
      }
    }
      ...
      if (loopBound && !lbe) {
         ...
         if (branches.first && branches.second) {
            if (fc > loopBound && tc > loopBound) {
               ...
               if (EncVer) llvm::errs() << "executingPM on successor second\n";
  ...
  case Instruction::Call: {
    ...
    if (f) {
       if (state.reachedFunctions.find(f->getName()) == state.reachedFunctions.end()) {
          ...
          RD* rdd = getrdmap(&state);
          if (rdd->HA) {
             if (EncVer) llvm::errs() << " Terminating path at timing observation point " << f->getName() 
    ...
    if (state.inEnclave && f && std::find(untrusted->begin(), untrusted->end(), f->getName()) != untrusted->end()) {
       ...
       terminateStateOnExit(*tobeterm);
       */ 
       if (EncVer) llvm::errs() << "making ocall " << f->getName() << " from " << ((Function*)i->getParent()->getParent())->getName() << "\n";
    ...
    if (isa<InlineAsm>(fp)) {
       ...
       if (symbolizeInlineAssembly) {
          ...
          if (EncVer) llvm::errs() << "inline assembly is unsupported: " << iasm->getAsmString() << "\n";
  ...
  case Instruction::Select: {
    ...
    if (BreakSelectStmt) {
       ...
       if (branches.first) {
          ...
          successorsPaths->insert(branches.first); // Keeping track of the new paths
         /* SYSREL side channel end */
          if (EncVer) llvm::errs() << "True branch of select: " << (*ki->inst) << " condition " << cond << "\n";
       }
       if (branches.second) {
          ...
          successorsPaths->insert(branches.second); // Keeping track of the new paths
          /* SYSREL side channel end */
          if (EncVer) llvm::errs() << "False branch of select: " << (*ki->inst) << " condition !" << cond << "\n";
       }
    }
    else {
       ref<Expr> cond = eval(ki, 0, state).value;
  ...
  case Instruction::GetElementPtr: {
    ...
    for (std::vector< std::pair<unsigned, uint64_t> >::iterator
      ...
      /* SYSREL extension */ 
      /* side channel begin */
      if (EncVer) llvm::errs() << "Before checking security sensitive, inEnclave?" << state.inEnclave << "\n";
      if (state.inEnclave && exprHasSymRegion(state, index, true)) {
          if (EncVer) llvm::errs() << "CRITICAL: Security sensitive index in GetElementPtr:\n";
          if (EncVer) ki->inst->print(llvm::errs());
          #ifdef VB
          if (EncVer) llvm::errs() << index << "\n"; 
          #endif
      }
      /* side channel end */
      /* SYSREL extension */ 
      ...
      #ifdef VB
      if (EncVer) llvm::errs() << "gep base: " << base << "\n";
      #endif
    }
    if (kgepi->offset) {
      /* SYSREL extension */ 
    ...
    #ifdef VB
    if (EncVer) llvm::errs() << "geptr final base: " << base << "\n";
    #endif
    ...
    if (PreferredResolution) {
       if (symIndexToMemBase.find((long)&state) != symIndexToMemBase.end()) {
           ...
           if (base != origbase || m.find(base) == m.end()) { 
              ...
              if (EncVer) llvm::errs() << "recording mapping from geptr index " << base << " to " << origbase << " in state " << &state << "\n";
          }
       }
    }
    ...
    if (state.inEnclave && exprHasSymRegion(state, base, true)) {
       ...
       if (result) {
          ...
          if (EncVer) llvm::errs() << "CACHE BASED SIDE CHANNEL: (MODE=" << mode << ")\n";
          if (EncVer) llvm::errs() << "Timing=" << statsTracker->elapsed() << "\n";
          if (EncVer) ki->inst->print(llvm::errs());
          const InstructionInfo &ii = kmodule->infos->getInfo(ki->inst);
          std::pair<std::string, int> p = std::pair<std::string, int>(ii.file, ii.line);
          cachelocs.insert(p);
          if (EncVer) llvm::errs() << "Stack trace for the code loc " << ii.file << " " << ii.line << "\n";
          if (EncVer) llvm::errs() << getStackTrace(state) << "\n";
          std::string tstr;
          llvm::raw_string_ostream ros(tstr);
          ros << (*(ki->inst)) << "\n";
          datacacheos << "Timing=" << statsTracker->elapsed() << "\n";
          datacacheos << "Instruction: " << ros.str() << "\n";
          datacacheos << "Location: " << ii.file.c_str() << " " << ii.line << "\n\n\n";
       }
    }
    #ifdef INFOFLOW
    ...
    if (t->isPointerTy()) {
       ...
       if (!t->isPointerTy()) {
          ...
          if (st) {
             ...
             #ifdef VB
             if (EncVer) llvm::errs() << "mapping bitcast instance of " << tname << " to " << result << " type pointer=" << t << " in " << ki->inst << "\n";
             #endif
          }
       }
    }
...
void Executor::run(ExecutionState &initialState) {
  ...
  /* SYSREL side channel begin */
  datacacheos.rdbuf()->pubsetbuf(0, 0);
  datacacheos.open("dataCacheSideChannels_time.txt", std::fstream::out);
  secretdependentos.rdbuf()->pubsetbuf(0, 0);
  secretdependentos.open("secretDependentBranch_time.txt", 
                                       std::fstream::out); 
  ...
  if (EncVer) llvm::errs() << "Registering publicOutputReturningFName as " << publicOutputReturningFName << "\n";
  ...
  if (usingSeeds) {
    ...
    while (!seedMap.empty()) {
      if (haltExecution) {
        if (EncVer) llvm::errs() << "Halting execution (seedmap not empty)!!!\n";        
        doDumpStates();
        return;
      }
    ...
    if (OnlySeed) {
      if (EncVer) llvm::errs() << "Halting execution (only seed) !!!\n";
      doDumpStates();
      return;
    }
  }
  ...
  if (EncVer) llvm::errs() << "Using searcher "; 
  if (EncVer) searcher->printName(llvm::errs());
  if (EncVer) llvm::errs() << "\n";
  ...
  if (sidechannelstarted) {
   ...
   if (EncVer) llvm::errs() << "inserting instruction " << (*ii) << " 's bb " << ii->getParent() << " to " << currentRD->stateid << "\n";  
   currentRD->bbs.insert((long)ii->getParent());

   if (isbranch) {
      if (EncVer) llvm::errs() << "it was a branch instr!\n";
      isbranch = false;
      if (HandleSingleSuccessors && successorsPaths->size() == 1) {
         if (EncVer) llvm::errs() << "Single successor on path:\n";
         //if (EncVer) ExprPPrinter::printConstraints(llvm::errs(), state.constraints);
         std::set<klee::ExecutionState*>::iterator it = successorsPaths->begin();
         bool done = false;
         for(; it!=successorsPaths->end(); ++it) {
            ExecutionState *s = *it;
            ref<Expr> r1 = s->lastCondition;
            if (EncVer) llvm::errs() << "last condition: " << r1 << "\n";
	 ...
	 for(; it!=successorsPaths->end(); ++it) {
 	    ...
 	    ref<Expr> r1 = s->lastCondition;
		    ...
		    if (EncVer) r1->dump();
                    const InstructionInfo &iit = kmodule->infos->getInfo(state.prevPC->inst);
                    if (EncVer) state.prevPC->inst->print(llvm::errs());
                    if (EncVer) llvm::errs() << "instruction in ii: " << (*ii) << "\n";
                    if (EncVer) printInfo(iit);                    
                    if (EncVer) llvm::errs() << "has high? " << hasHloc << "\n";
                    if (EncVer) llvm::errs() << "has low? " << hasLloc << "\n";
             //#endif
             if (hasHloc)
                s->setSecretDescendant(true);

             RD* srd = newNode(s);
             if (EncVer) llvm::errs() << "instruction recorded to " << srd->stateid << " " << (*srd->i) << "\n";
	     s->rdid = srd->stateid;
	     ...
	     if (hasHloc && !hasLloc) {
	     	ref<Expr> proj = getProjectionOnRegion(state, r1, true);
                if (EncVer) llvm::errs() << " high projection on " << r1 << " : " << proj << "\n";
                ...
                if (result) {
                   if (EncVer) llvm::errs() << "Timing=" << statsTracker->elapsed() << "\n";
                   //#ifdef VBSC
                          if (EncVer) std::cerr << "Projection on high consistent with PC\n";
    	                  if (EncVer) std::cerr << "\n>>>> Projection : ";
	                  if (EncVer) proj->dump();
		   ...
		   if (!done) {
		      ...
		      rdd->numSucc = successorsPaths->size();
	              ...
	              printsucc = true;
                      ...
                      if (EncVer) llvm::errs() << "recorded security sensitive branch " << ss.str() << "\n";
                      secretdependentos << "Time=" << statsTracker->elapsed() << "\n";
                      secretdependentos << "Instruction: " << ros.str() << "\n";
		   ...
		   srd->LC = klee::ConstantExpr::create(true, 1);
                ...
                if (hasHloc && hasLloc) {
                   ...
                   #ifdef VBSC
                   if (EncVer) llvm::errs() << "Mixed Branch Condition:\n";
  ...
  searcher = 0;
  // SYSREL side channel
  if (EncVer) std::cerr << "Size of rdmap : " << rdmap->size() << "\n";
  if (!SolverImpl::interrupted || !SolverImpl::forceOutput) {
    if (EncVer) llvm::errs() << "Halting execution from regular loop!\n";
    doDumpStates();
  }
  /* SYSREL side channel begin */
...
void Executor::terminateState(ExecutionState &state) {
  ...
  #ifdef VB
  if (EncVer) llvm::errs() << "Checking the state at the end of path\n";
  state.check();
  #endif
  /* SYSREL extension */
...
void Executor::callExternalFunction(ExecutionState &state,
 ...
 if (!APIHandler::handles(function->getName()) && !isInfoFlowAPI(removeDotSuffix(function->getName()))) { // this check is a SYSREL extension to avoid issues with pointers to 128 bit 
  if (EncVer) llvm::errs() << "treating as external function: " << function->getName() << "\n";
 ...
 /* SYSREL EXTENSION */
  ...
  if (!success) {
    ...
    if (progModel) {
      ...
      if (EncVer) llvm::errs() << "before check 1\n";
      symbolizeAndMarkSensitiveArgumentsOnCall(state, target, function, arguments, true);
      if (EncVer)  llvm::errs() << "after check 1\n";
      // check if PROSE version of the function exists, if so use that one
      std::string prosename = function->getName().str() + "_PROSE"; 
      Function *proseFn = moduleHandle->getFunction(prosename);
      if (proseFn) {
         if (EncVer) llvm::errs() << "WARNING: Using " << prosename << " for " << function->getName() << "\n";
       ...
       if (!resHandled) {
          ...
          for(auto pr: prefixRedact) {
             if (function->getName().find(pr) == 0) {
                ...
                if (EncVer) llvm::errs() << "Checking model function " << modelName << " using redacted prefix " << pr << "\n";
                if (modelFn) {
                   if (EncVer)  llvm::errs() << "WARNING: Using " << modelName << " for " << function->getName() << "\n";
          ...
          if (UseInternal) {
             Function *inf = getInternalVersion(function->getName().str());
             if (inf) {
                if (EncVer) llvm::errs() << "WARNING: Using internal function " << inf->getName() << " for function " << function->getName() << "\n";
                ((CallInst*)target->inst)->setCalledFunction(inf);
                executeCall(state, target, inf, arguments);   
                return;
             }
          }
...
bool Executor::symbolizeAndMarkSensitiveArgumentsOnCall(ExecutionState &state,
    ...
    if (EncVer) llvm::errs() << "Handling functions with sensitive args " << function->getName() << "\n"; 
    unsigned int ai = 0;
    for(llvm::Function::arg_iterator agi = function->arg_begin(); agi != function->arg_end(); agi++, ai++) {
       if (argsH.find(ai) != argsH.end() || argsL.find(ai) != argsL.end() || argsM.find(ai) != argsM.end()) {
          if (EncVer) llvm::errs() << "handling arg " << ai << " " << arguments[ai] << "\n"; 
          // a local var
          Type *at = agi->getType();
          bool pointerType = false;
          if (at->isPointerTy()) {
                ...
                if (cexpr) {
                  ...
                  if (asuccess) {
                   if (state.isSymbolic(op.first) || updatedWithSymbolic.find((long)op.first) != updatedWithSymbolic.end()) {
                      ...
                      if (!CE) {
                        if (EncVer) llvm::errs() << "going to check sym region sensitive\n";
                        if (isSymRegionSensitive(state, cur, fname, ai, true)) {
                           if (EncVer) llvm::errs() << cur << "already known to be sensitive \n";
                           continue;
                        }
                        if (EncVer) llvm::errs() << "done checking sym region sensitive\n";
                     } 
                     }
                     // set high/low
                     ...
                     if (EncVer) llvm::errs() << &state << " calling setSymRegionSensitive from OnCall " << fname  << " for arg " << ai << "\n";
                   ...
                   if (EncVer) llvm::errs() << &state << " calling setSymRegionSensitive from OnCall 2" << fname  << " for arg " << ai << "\n";
                   setSymRegionSensitive(state, op.first, fname, bt, ai, pointerType, true, diff);
                  }
                 }
              }
           ...
           if (!useCallInstOperands) {
            ...
            if (EncVer) llvm::errs() << &state << " calling setSymRegionSensitive from OnCall 3" << fname  << " for arg " << ai << "\n";
            setSymRegionSensitive(state,mo, fname, at, ai, pointerType, true);
             ...
             if (EncVer) llvm::errs() << "Skipping marking sensitivity for arg " << ai << " with value " << arguments[ai] << " in function " << function->getName() << " to avoid data-flow analysis\n";
          }
         }
      }
    }
...
void Executor::setSymRegionSensitive(ExecutionState &state,
   ...
   if (argsH.find(ai) != argsH.end()) {
      std::vector<region> rs; 
      if (EncVer) llvm::errs() << "base address of high marked memory: " <<  sm->getBaseExpr() << "\n";
      if (highTypeRegions.find(btname) != highTypeRegions.end()) {
         ...
         r.offset = offset;
         //r.size = sm->size * 8;
         if (EncVer) llvm::errs() << "setting region size of " << sm->name << " to " << argsize * 8 << " while memobj size is " << sm->size * 8 << "\n";
         if (pointerType)
            r.size = sm->size * 8;
         else 
            r.size = argsize * 8;
      ...
      if (EncVer) llvm::errs() << &state << " calling setHighSym from setSymRegionSensitive for " << fname  << " for arg " << ai << "\n";
      ...
      if (!hintFound && lowTypeRegions.find(btname) == lowTypeRegions.end() && 
         ...
         if (EncVer) llvm::errs() << "type=" << btname << "\n"; 
      ...
      if (lowTypeRegions.find(btname) != lowTypeRegions.end()) {
       ...
       if (highTypeRegions.find(btname) != highTypeRegions.end()) {
          ...
          setHighMemoryRegion(state, sm->getBaseExpr(), rs);
          if (EncVer) llvm::errs() << "calling setHighSym from setSymRegionSensitive for " << fname  << "\n";
          setHighSymRegion(sm->name, rs); 
       }
   }
}
...
bool Executor::symbolizeAndMarkArgumentsOnReturn(ExecutionState &state,
                                  ...
                                  ref<Expr> ret_value) {
    std::string fname = removeDotSuffix(function->getName());    
    if (EncVer) llvm::errs() << "state :" << &state << " checking " << fname << " to see if an input function\n";
    ...
    if (!function->getReturnType()->isVoidTy()) {
       ...
       if (CE) {
          if (CE->getZExtValue() != success_value) {
             if (EncVer) llvm::errs() << "skipping input function " << fname << " handling due to error path\n";
             return true;
          }
          tstate = &state;
       }
          ...
          if (branches.first) {
             ...
             if (EncVer) llvm::errs() << "skipping input function " << fname << " handling due to not satisfying success case:\n";
             return true;
          }
       }
    }
    ...
    if (mixedFunctionArgsRet.find(fname) != mixedFunctionArgsRet.end())
       argsM = mixedFunctionArgsRet[fname];     
    if (EncVer) llvm::errs() << "state: " << tstate << " Handling input function " << function->getName() << "\n"; 
    unsigned int ai = 0;
    for(llvm::Function::arg_iterator agi = function->arg_begin(); agi != function->arg_end(); agi++, ai++) {
       if (argsH.find(ai) != argsH.end() || argsL.find(ai) != argsL.end() || argsM.find(ai) != argsM.end()) { 
          // a local var
          ...
          if (at->isPointerTy()) {
                ...
                if (asuccess) {
                  if (tstate->isSymbolic(op.first) || updatedWithSymbolic.find((long)op.first) != updatedWithSymbolic.end()) {
                     ...
                     if (cexpr && mob) 
                        diff = cexpr->getZExtValue() - mob->getZExtValue(); 
                     if (EncVer) llvm::errs() << "calling setSymRegionSensitive from OnReturn " << fname  << " btname= " << btname << "\n";
                   ...
                   //#ifdef VB
                   if (EncVer) llvm::errs() << "Symbolizing and marking argument " << ai << " of function " << function->getName() 
                                << ", address=" << sm->getBaseExpr() << "\n";
                   if (EncVer) llvm::errs() << "base addres=" << base << "\n";
                   //#endif
                   std::string name = function->getName();
                   std::string uniqueName = name;
                   if (argsH.find(ai) != argsH.end())
                   ...
                   if (co) {
                      if ((op.first->size - co->getZExtValue()) < sm->size) {
                         //#ifdef VB
                         if (EncVer) llvm::errs() << "Lazy init of a void pointer mismatches real type, "
       ...
       if (target->operands[ai+1] >= 0) { // a local var
          ...
          if (at->isPointerTy()) {
             ...
             //if (bt->getPrimitiveSizeInBits()) {
                ...
                solver->setTimeout(0);
                if (asuccess) {
                   if (EncVer) llvm::errs() << "Computing type info on" << (*target->inst) 
                                << " for type " << rso.str() << "\n";
                   MemoryObject *sm = memory->allocate(TD->getTypeAllocSize(bt), op.first->isLocal,
                           op.first->isGlobal, op.first->allocSite, TD->getPrefTypeAlignment(bt));
                   recordMemObj(state, sm);
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    //#ifdef VB
    if (isLocal)
       if (EncVer) llvm::errs() << "Local alloc: " << (*target->inst) << " at address " << mo->getBaseExpr() << "\n"; 
    //#endif
   ...
   if (infoFlowSummaryMode && isLocal && target->dest >= 0)  {
      if (EncVer) llvm::errs() << "recording register " << target->dest << " as a local variable\n"; 
   ...
   if (lazyInit) {
    ...
    if (EncVer) llvm::errs() << "Inferred alloc size " << example << "\n";
    ...
    if (fixedSize.second) {
      ...
      if (res) {
        ...
        if (hugeSize.second) {
          ...
          /* SYSREL extension */
          if (lazyInit) {
             if (EncVer) llvm::errs() << "concretized symbolic size as " << primArraySize << "\n";
    ...
    if (fixedSize.first) { // can be zero when fork fails
       ...
       if (EncVer) llvm::errs() << "Encountered a large alloc; using the primitive array size instead!\n";
       ref<Expr> ps = ConstantExpr::alloc(primArraySize, 32);
...
bool Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (reached.find(state.prevPC->inst->getParent()->getParent()->getName()) == reached.end()) { 
     if (EncVer) llvm::errs() << "reached " << state.prevPC->inst->getParent()->getParent()->getName() << "\n";
  ...
  if (EncVer) llvm::errs() << "address width " << address->getWidth() << "\n";
  ...
  if (!state.addressSpace.resolveOne(state, solver, address, op, success)) {
    ...
    if (EncVer) llvm::errs() << "address2 width " << address->getWidth() << "\n"; 
  ...
  if (success) {
    ...
    //if (forcedOffset) {
       ...
       //#ifdef VB
       if (EncVer) llvm::outs() << "(forced) offset for target address " << offset << "\n";
      //#endif
    //}
    /* SYSREL EXTENSION */
    ...
    if (inBounds) {
      ...
      if (isWrite) {
        if (os->readOnly) {
          ...
          /* SYSREL EXTENSION */
          if (CheckInfoFlowRule) {

             // if value is labeled high and rExpr is labeled low, report violation 
             bool hasHloc = exprHasSymRegion(state, value, true);
             ...
             if (hasHloc)
                if (EncVer) llvm::errs() << "HIGH expr: " << value << " const offset?" << CO << "\n";


             if (hasHloc && CO && (os->isByteConcrete(CO->getZExtValue()) || 
                                os->isByteKnownSymbolic(CO->getZExtValue()))) {
                ref<Expr> rExpr = os->read(offset, type);
                if (EncVer) llvm::errs() << "going to check low label!\n";
                if (exprHasSymRegion(state, rExpr, false))  {
                   leakage = true;                
                   if (EncVer) llvm::errs() << "source: " << value << "\n";
                   if (EncVer) llvm::errs() << "dest: " << rExpr << "\n";
                }
             }   
             /*if (hasHloc && isLowMemoryRegion(state, mo->getBaseExpr())) 
                leakage = true;  */

             if (leakage) {
                llvm::errs() << " INFORMATION FLOW ERROR: HIGH FLOWS INTO LOW!!!\n";
                const InstructionInfo &ii = kmodule->infos->getInfo(target->inst);
                target->inst->print(llvm::errs());
                printInfo(ii);
                std::string MsgString;
                llvm::raw_string_ostream msg(MsgString);
                state.dumpStack(msg);
       ...
       if (lazyInit) {
        if (!dyn_cast<ConstantExpr>(result)) {
            ...
            if (li) {
               ...
               //#ifdef VB 
               if (EncVer) llvm::errs() << "Is " << rsostr << " (count=" << count << ") to be lazy init?\n";
               inst->dump();
               //#endif
               if (lazyInitTemp) {
                  if (t->isPointerTy()) {
                 ...
                 //#endif
                  ...
                  //#ifdef VB
                  if (EncVer) llvm::errs() << "Allocating memory for type " << rso2str << " of size " << "\n";
                  //#endif
                  ref<Expr> laddr;
                  llvm::Type *rType;
                  bool mksym;
                  ...
                  //#ifdef VB
                  if (EncVer) llvm::errs() << "lazy init mem obj addr=" << laddr << " in state " << &state << " count=" << count << "\n";
                  //#endif
                  ...
                  // Todo: Consider applying the initializations starting at an offset to handle embeddings
                  if (getTypeName(t) == getTypeName(rType))
                     initializeLazyInit(mo,wos,t);
                  else if (EncVer) llvm::errs() << "WARNING: Missed a chance to initialize during lazy init due to embedding!\n"; 

                  std::pair<unsigned, unsigned> fieldInfo;
                  Type *enclosingType = getEnclosingObjectType(target->inst);
                  if (enclosingType) { 
                     if (EncVer) llvm::errs() << "enclosing type inferred from IR: " << getTypeName(enclosingType) << "\n";
                     ...
                     if (klee::ConstantExpr *OCE = dyn_cast<klee::ConstantExpr>(offset)) {
                        if (getDataConstraint(etname, OCE->getZExtValue(), fieldInfo)) {
                           ...
                           if (EncVer) llvm::errs() << "Adding data constraint " << dcons << "\n"; 
                        }
                        if (EncVer) llvm::errs() << "Checking if type " << etname << " to be zero init at offset " << OCE->getZExtValue() << "\n";
                        if (getZeroInitArray(etname, OCE->getZExtValue())) {
                           //const ObjectState *mos = state.addressSpace.findObject(mo);
                           if (mos) {
                              if (EncVer) llvm::errs() << "Zeroing array at offset " << OCE->getZExtValue() 
                                           << " of type " << etname << "\n";
                              //ObjectState *wos = state.addressSpace.getWriteable(mo, mos);
                  ...
                  //#ifdef VB
                  if (EncVer) llvm::errs() << "lazy initializing writing " << laddr << "( inside " << mo->getBaseExpr() << ") to " << address << " in state " << &state << "\n";
                  //#endif
                  forcedOffset = true;
        ...
        //#ifdef VB
        if (getDestCell(state, target).value.get())
         if (EncVer) llvm::errs() << "address to be dest: " << getDestCell(state, target).value << "\n";
        //#endif
  ...
  if (PreferredResolution) { // use the base address stored in getelementptr to use the MemoryObject for resolution
     if (EncVer) llvm::errs() << "Checking if we can identify the single resolution candidate with PreferredResolution option!\n"; 
     if (symIndexToMemBase.find((long)&state) != symIndexToMemBase.end()) {
          std::map<ref<Expr>, ref<Expr> > m = symIndexToMemBase[(long)&state];
          ref<Expr> addtocheck = address;
          int i=0;
          while (m.find(addtocheck) != m.end() && i++ < 3) {
             if (EncVer) llvm::errs() << " checking mapping for " << addtocheck << "\n";
             ...
             if (addressToMemObj.find((long)&state) != addressToMemObj.end()) {
                ...
                if (m2.find(mobase) != m2.end()) {
                   ...
                   if (mres && ores) {
                      ...
                      rl.push_back(opres);
                      craftedResList = true; 
                      if (EncVer) llvm::errs() << "Using mem obj with base " << mobase << " to resolve " << address << "\n";
                      break;
                   } 
                   else break;
                }
                   ...
                   if (EncVer) llvm::errs() << "memobj for " << mobase << " in state " << &state << " not found \n"; 
                   addtocheck = mobase;                   
                }
             }
             else break;
  ...
  for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
    ...
    if (bound) {
      ...
      if (EncVer) llvm::errs() << "inbound case MemObj base=" << mo->getBaseExpr() << " mo size=" << mo->size << " PC:\n";
      if (EncVer) ExprPPrinter::printConstraints(llvm::errs(), bound->constraints);
      ...
      if (isWrite) {
        if (os->readOnly) {
          ...
          if (forcedOffset)
             ...
             if (EncVer) llvm::errs() << "case 2: " << target << "\n";
  ...
  if (unbound) {
    if (incomplete) {
      ...
      #ifdef VB
      if (EncVer) llvm::outs() << "Offending address for unbound" << address << "\n";
      if (EncVer) ExprPPrinter::printConstraints(llvm::errs(), unbound->constraints);
      #endif
...
void Executor::initArgsAsSymbolic(ExecutionState &state, Function *entryFunc, bool &abort, bool nosym) {
   ...
   for(llvm::Function::arg_iterator ai = entryFunc->arg_begin(); ai != entryFunc->arg_end(); ai++, argi++) {
     ...
     llvm::raw_string_ostream rso(type_str);
     at->print(rso);
     if (EncVer) llvm::errs() << "base type pointer=" << at << "\n";
     if (at->isPointerTy()) {
        ...
        //#ifdef VB
        if (EncVer) llvm::errs() << "arg " << ind << " type " << getTypeName(at) << " pointer= " << at << " count=" << count << "\n";
        //#endif
        //if (lazyInitT) {
          /*
           ...
           */
              ...
              if (abort) return;
              //#ifdef VB
              if (EncVer) llvm::errs() << "Symbolizing arg " << argi << " of " << entryFunc->getName() << ", address " << mo->getBaseExpr() << " size=" << mo->size << "\n";
              //mo = memory->allocateForLazyInit(state, state.prevPC->inst, at, singleInstance, count, laddr);
              if (EncVer) llvm::errs() << "is arg " << ind <<  " type " << rso.str() << " single instance? " << singleInstance << "\n";
              if (EncVer) llvm::errs() << "to be made symbolic? " << mksym << "\n";
              //#endif
              ...
              if (argH.find(argi) != argH.end()) {
                 ...
                 if (highTypeRegions.find(atname) != highTypeRegions.end()) {
                    ...
                    r.size = mo->size * 8;
                    rs.push_back(r); 
                    if (EncVer) llvm::errs() << " setting high security region offset= " << r.offset << " size= " << r.size << "\n";
                 }
                 mo->ishigh = true;
                 setHighMemoryRegion(state, mo->getBaseExpr(), rs);
                 setHighSymRegion(mo->name, rs);
                 //#ifdef VB
                 if (EncVer) llvm::errs() << "recording address of " << mo->name << " " << mo->getBaseExpr() << " as high security sensitive\n";
                 //#endif 
              }
                 ...
                 if (lowTypeRegions.find(atname) != lowTypeRegions.end()) {
                    ...
                    r.size = mo->size * 8;
                    rs.push_back(r);
                    if (EncVer) llvm::errs() << " setting low security region offset= " << r.offset << " size= " << r.size << "\n";
                 }
                 mo->islow = true;
                 setLowMemoryRegion(state, mo->getBaseExpr(), rs);
                 setLowSymRegion(mo->name, rs);
                 //#ifdef VB
                 if (EncVer) llvm::errs() << "recording address of " << mo->name << " " << mo->getBaseExpr() << " as low security insensitive\n";
                 //#endif 
              }
                 ...
                 if (highTypeRegions.find(atname) != highTypeRegions.end()) {
                    ...
                    setHighSymRegion(mo->name, rs); 
                    //#ifdef VB
                    if (EncVer) llvm::errs() << "recording address of " << mo->name << " " << mo->getBaseExpr() << " as high security insensitive\n";
                    //#endif            
                 }
              }
              if (!nosym/* && mksym*/) {
       ...
       // else llvm::outs() << "skipping lazy init for " << rso.str() << "\n";
        ...
        recordMemObj(state, mo);
        mo->name = getUniqueSymRegion(fname  + std::string("_arg_") + std::to_string(ind));
        if (EncVer) llvm::errs() << "lazy init arg " << mo->name << "\n";
        // we're mimicking what executeMemoryOperation do without a relevant load or store instruction
        const Array *array = arrayCache.CreateArray(mo->name, mo->size);
        ObjectState *sos = bindObjectInState(state, mo, true, array);
        state.addSymbolic(mo, array);
        ...
        if (argH.find(argi) != argH.end()) {
           ...
           setHighSymRegion(mo->name, rs);
           //#ifdef VB
            if (EncVer) llvm::errs() << "recording address of " << mo->name << " " << mo->getBaseExpr() << " as security sensitive \n";
           //#endif 
        }
        else if (argL.find(argi) != argL.end()) {
           std::vector<region> rs;
           ...
           setLowSymRegion(mo->name, rs);
           //#ifdef VB
            if (EncVer) llvm::errs() << "recording address of " << mo->name << " " << mo->getBaseExpr() << " as not security sensitive\n"; 
           //#endif 
        }        
        else if (argM.find(argi) != argM.end()) {
           if (lowTypeRegions.find(atname) == lowTypeRegions.end() && 
